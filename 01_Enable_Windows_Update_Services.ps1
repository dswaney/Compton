...
# GitHub self-update settings for the 00 updater script
$UpdaterRepoOwner    = 'dswaney'
$UpdaterRepoName     = 'Compton'
$UpdaterRepoBranch   = 'main'
$UpdaterRepoSubFolder = ''
$UpdaterScriptFile   = '00_Update-Scripts-FromGitHub.ps1'

function Get-Utf8FileTextSafe {
    param(
        [Parameter(Mandatory)]
        [string]$Path
    )
...
function Backup-FileSafe {
    ...
    $backupPath = Join-Path $backupFolder ("${baseName}_${timestamp}${extension}.bak")
    Copy-Item -LiteralPath $Path -Destination $backupPath -Force
    Write-Status "Backed up [$Path] to [$backupPath]" 'OK'
    return $backupPath
}

function Get-UpdaterRawGitHubUrl {
    param(
        [Parameter(Mandatory)]
        [string]$FileName
    )

    $pathPart = if ([string]::IsNullOrWhiteSpace($UpdaterRepoSubFolder)) {
        $FileName
    }
    else {
        ($UpdaterRepoSubFolder.Trim('/').Replace('\\','/') + '/' + $FileName)
    }

    return ('https://raw.githubusercontent.com/{0}/{1}/{2}/{3}' -f $UpdaterRepoOwner, $UpdaterRepoName, $UpdaterRepoBranch, $pathPart)
}

function Get-RemoteUpdaterContent {
    param(
        [Parameter(Mandatory)]
        [string]$FileName
    )

    $url = Get-UpdaterRawGitHubUrl -FileName $FileName
    ...
    try {
        ...
        $response = Invoke-WebRequest -Uri $finalUri `
                                      ...
                                      -Headers @{
                                          ...
                                          'Pragma'        = 'no-cache'
                                          'User-Agent'    = 'PowerShell-Compton-EnableScript'
                                      } `
                                      -ErrorAction Stop
...
function Get-PreferredUpdaterLocalPath {
    $candidatePaths = New-Object System.Collections.Generic.List[string]

    if (-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) {
        $candidatePaths.Add((Join-Path $PSScriptRoot $UpdaterScriptFile)) | Out-Null
    }

    if (-not [string]::IsNullOrWhiteSpace($PSCommandPath)) {
        $scriptFolder = Split-Path -Path $PSCommandPath -Parent
        if (-not [string]::IsNullOrWhiteSpace($scriptFolder)) {
            $candidatePaths.Add((Join-Path $scriptFolder $UpdaterScriptFile)) | Out-Null
        }
    }

    $candidatePaths.Add((Join-Path 'C:\Scripts' $UpdaterScriptFile)) | Out-Null
...
function Update-LocalUpdaterScriptFromGitHub {
    [CmdletBinding()]
    param()
    ...
    try {
        ...
        $localUpdaterPath = Get-PreferredUpdaterLocalPath
        Write-Status "Checking GitHub for updated copy of [$UpdaterScriptFile]..." 'INFO'
        Write-Status "Target local updater path: $localUpdaterPath" 'INFO'

        $remoteContent = Get-RemoteUpdaterContent -FileName $UpdaterScriptFile
        $remoteVersionText = Get-ScriptHeaderValue -Content $remoteContent -HeaderName 'ScriptVersion'
        $remoteVersion = Convert-ToVersionObjectSafe -VersionText $remoteVersionText

        if ([string]::IsNullOrWhiteSpace($remoteVersionText)) {
            throw "Remote updater file is missing or has an unreadable '# ScriptVersion:' header."
        }

        $localContent = Get-Utf8FileTextSafe -Path $localUpdaterPath
        if ($null -eq $localContent) {
            $localVersionText = $null
            $localVersion = [version]'0.0'
            Write-Status "Local updater script not found. A fresh copy will be downloaded." 'WARN'
        }
        else {
        ...
        Write-Status "Updater local version: [$localVersionText] | GitHub version: [$remoteVersionText]" 'INFO'

        if ($remoteVersion -gt $localVersion) {
            $parentFolder = Split-Path -Path $localUpdaterPath -Parent
            if (-not (Test-Path -LiteralPath $parentFolder)) {
                New-Item -Path $parentFolder -ItemType Directory -Force | Out-Null
            }

            $backupPath = Backup-FileSafe -Path $localUpdaterPath
            Save-Utf8NoBom -Path $localUpdaterPath -Content $remoteContent

            if ($backupPath) {
                Write-Status "Updated [$UpdaterScriptFile] from [$localVersionText] to [$remoteVersionText]. Backup: [$backupPath]" 'OK'
            }
            else {
                Write-Status "Downloaded new local copy of [$UpdaterScriptFile] version [$remoteVersionText]." 'OK'
            }
        }
        else {
            Write-Status "Local [$UpdaterScriptFile] is current. No update needed." 'OK'
        }
    }
    catch {
        Write-Status "Updater self-check failed for [$UpdaterScriptFile] : $($_.Exception.Message)" 'WARN'
    }
}
...
Update-LocalUpdaterScriptFromGitHub
