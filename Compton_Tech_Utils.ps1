...
function Initialize-System {
    ...
    try {
        ...
        # MISAdmin validation is intentionally not performed during startup.
        # Use Option 1 when the local MISAdmin account needs to be created or verified.
...
function Remove-BloatwareApps {
    ...
    Windows Copilot and request Microsoft Copilot app removal on supported Windows 11 builds.
    #>
    ...
    $logPath = Join-Path $logRoot ("{0}-Option2-BloatwareRemoval-{1}.log" -f $env:COMPUTERNAME, $started.ToString('yyyy-MM-dd_HHmmss'))
    $script:Option2LogEntries = New-Object System.Collections.Generic.List[string]

    function Write-Option2Log {
        param(
        ...
        $script:Option2LogEntries.Add($line) | Out-Null
    ...
    function Set-Option2RegistryDword {
        param(
        ...
        try {
            ...
            Write-Option2Log ("Applied policy: {0} ({1}\{2}={3})" -f $Description, $Path, $Name, $Value) 'OK'
            return $true
        }
        catch {
            Write-Option2Log ("Failed policy: {0}: {1}" -f $Description, $_.Exception.Message) 'ERROR'
            return $false
        }
    }

    function Get-Option2DefaultBloatwarePatterns {
        [CmdletBinding()]
        param()

        # These are package name patterns. Wildcards are intentional for package-family variations.
        [ordered]@{
            'Microsoft Consumer / Inbox Apps' = @(
                'Microsoft.BingNews',
    ...
    function Get-Option2AppxPackagesByPattern {
        ...
        try {
            ...
            Write-Option2Log ("Failed to query AppX packages for pattern {0}: {1}" -f $Pattern, $_.Exception.Message) 'WARN'
            @()
        }
    }

    function Get-Option2ProvisionedPackagesByPattern {
        param([Parameter(Mandatory)][string]$Pattern)
        ...
        try {
            ...
            Write-Option2Log ("Failed to query provisioned packages for pattern {0}: {1}" -f $Pattern, $_.Exception.Message) 'WARN'
            @()
        }
    }

    function Test-Option2ProtectedAppxPackage {
        <#
        .SYNOPSIS
        Returns $true for protected Windows SystemApps that should be reported and skipped.

        .DESCRIPTION
        Some packages match consumer wildcard patterns but are part of Windows and cannot be
        removed with Remove-AppxPackage. Attempting removal generates noisy 0x80073CFA / 0x80070032
        failures. These packages are intentionally preserved and logged as SKIP instead of WARN/ERROR.
        #>
        [CmdletBinding()]
        param(
        ...
        try {
            if ($null -ne $Package.NonRemovable) {
        ...
        if ($protectedNames | Where-Object { $packageFullName -like ("{0}_*" -f $_) }) {
            return $true
        }
        ...
        # Do not broadly remove Windows SystemApps when they are matched only by a wildcard. Many are OS components.
        if ($installLocation -match '^[A-Za-z]:\\Windows\\SystemApps\\') {
            return $true
        }
    ...
    function Remove-Option2InstalledAppxPackage {
        param(
            [Parameter(Mandatory)]$Package,
        ...
        try {
            if ($PSCmdlet.ShouldProcess($Package.PackageFullName, 'Remove installed AppX package')) {
                Remove-AppxPackage -Package $Package.PackageFullName -ErrorAction Stop
            }
            Write-Option2Log ("Removed installed AppX package: {0}" -f $Package.PackageFullName) 'OK'
            return $true
        }
        catch {
            Write-Option2Log ("Failed to remove installed AppX package {0} matched by {1}: {2}" -f $Package.PackageFullName, $Pattern, $_.Exception.Message) 'WARN'
            return $false
        }
    }

    function Remove-Option2ProvisionedAppxPackage {
        param(
            [Parameter(Mandatory)]$Package,
            [Parameter(Mandatory)][string]$Pattern
        )
        ...
        try {
            ...
            Write-Option2Log ("Removed provisioned AppX package: {0}" -f $Package.PackageName) 'OK'
            return $true
        }
        catch {
            Write-Option2Log ("Failed to remove provisioned AppX package {0} matched by {1}: {2}" -f $Package.PackageName, $Pattern, $_.Exception.Message) 'WARN'
            return $false
        }
    }

    function Disable-Option2Copilot {
        [CmdletBinding(SupportsShouldProcess)]
        param()

        Write-Option2Log 'Applying Microsoft Copilot disable and removal controls.' 'INFO'

        # Legacy Windows Copilot policy. Still useful on older Windows 11 builds.
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot' -Name 'TurnOffWindowsCopilot' -Value 1 -Description 'Turn off Windows Copilot for machine' | Out-Null
        Set-Option2RegistryDword -Path 'HKCU:\Software\Policies\Microsoft\Windows\WindowsCopilot' -Name 'TurnOffWindowsCopilot' -Value 1 -Description 'Turn off Windows Copilot for current user' | Out-Null

        # WindowsAI policy values used by newer Windows 11 management baselines / CSP-backed policies.
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsAI' -Name 'RemoveMicrosoftCopilotApp' -Value 1 -Description 'Request Microsoft Copilot app removal on supported Windows 11 builds' | Out-Null
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsAI' -Name 'TurnOffWindowsCopilot' -Value 1 -Description 'Turn off Windows Copilot WindowsAI machine policy' | Out-Null
        Set-Option2RegistryDword -Path 'HKCU:\Software\Policies\Microsoft\Windows\WindowsAI' -Name 'RemoveMicrosoftCopilotApp' -Value 1 -Description 'Request Microsoft Copilot app removal for current user on supported Windows 11 builds' | Out-Null
        Set-Option2RegistryDword -Path 'HKCU:\Software\Policies\Microsoft\Windows\WindowsAI' -Name 'TurnOffWindowsCopilot' -Value 1 -Description 'Turn off Windows Copilot WindowsAI current-user policy' | Out-Null

        # Disable newer AI entry points where the policy value exists on supported builds.
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsAI' -Name 'DisableClickToDo' -Value 1 -Description 'Disable Click to Do where supported' | Out-Null
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsAI' -Name 'DisableCocreator' -Value 1 -Description 'Disable Paint Cocreator where supported' | Out-Null
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsAI' -Name 'DisableGenerativeFill' -Value 1 -Description 'Disable Paint generative fill where supported' | Out-Null
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsAI' -Name 'DisableImageCreator' -Value 1 -Description 'Disable Paint Image Creator where supported' | Out-Null

        # Hide taskbar Copilot button for default/current shell policy paths when present.
        Set-Option2RegistryDword -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced' -Name 'ShowCopilotButton' -Value 0 -Description 'Hide Copilot taskbar button for current user' | Out-Null
        Set-Option2RegistryDword -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced' -Name 'ShowCopilotButton' -Value 0 -Description 'Hide Copilot taskbar button machine default' | Out-Null

        try {
            $copilotProcesses = @(Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.Name -like '*Copilot*' })
            foreach ($process in $copilotProcesses) {
                if ($PSCmdlet.ShouldProcess($process.ProcessName, 'Stop Copilot process')) {
                    Stop-Process -Id $process.Id -Force -ErrorAction Stop
                }
                Write-Option2Log ("Stopped Copilot process: {0} ({1})" -f $process.ProcessName, $process.Id) 'OK'
            }
        }
        catch {
            Write-Option2Log ("Failed while stopping Copilot processes: {0}" -f $_.Exception.Message) 'WARN'
        }
        ...
        try {
            ...
            foreach ($task in $copilotTasks) {
                if ($PSCmdlet.ShouldProcess(("{0}{1}" -f $task.TaskPath, $task.TaskName), 'Disable Copilot scheduled task')) {
                    Disable-ScheduledTask -TaskName $task.TaskName -TaskPath $task.TaskPath -ErrorAction Stop | Out-Null
                }
                Write-Option2Log ("Disabled Copilot scheduled task: {0}{1}" -f $task.TaskPath, $task.TaskName) 'OK'
            }
        }
        catch {
            Write-Option2Log ("Failed while disabling Copilot scheduled tasks: {0}" -f $_.Exception.Message) 'WARN'
        }
    }
    ...
    $results = [ordered]@{
        ...
        InstalledPackagesNotFound = 0
        FailedRemovals            = 0
        CopilotPoliciesApplied     = $false
        ...
        SkippedProtectedItems     = New-Object System.Collections.Generic.List[string]
        LogPath                   = $logPath
    }

    try {
        Write-Option2Log '=== Option 2 Windows 11 Bloatware and Copilot Removal Started ===' 'INFO'
        Write-Option2Log ("Computer: {0}" -f $env:COMPUTERNAME) 'INFO'
        Write-Option2Log ("Run mode: {0}" -f $(if ($WhatIfPreference) { 'WhatIf / simulation' } else { 'Apply changes' })) 'INFO'

        if (-not $SkipCopilotDisable) {
            Disable-Option2Copilot
            $results.CopilotPoliciesApplied = $true
        }
        else {
            Write-Option2Log 'Copilot disable policy step skipped by parameter.' 'SKIP'
        }

        $patterns = if ($CustomAppList -and $CustomAppList.Count -gt 0) {
            [ordered]@{ 'Custom List' = $CustomAppList }
        }
        else {
            Get-Option2DefaultBloatwarePatterns
        }
        ...
        foreach ($category in $patterns.Keys) {
            Write-Option2Log ("Processing category: {0}" -f $category) 'INFO'

            foreach ($pattern in $patterns[$category]) {
                $results.PatternsProcessed++
                Write-Progress -Activity 'Option 2 - Removing Windows bloatware' -Status ("Processing {0}" -f $pattern) -PercentComplete 0

                $installedPackages = @(Get-Option2AppxPackagesByPattern -Pattern $pattern)
                if ($installedPackages.Count -eq 0) {
                    $results.InstalledPackagesNotFound++
                    Write-Option2Log ("No installed AppX package found for pattern: {0}" -f $pattern) 'SKIP'
                }
                else {
                    foreach ($package in $installedPackages) {
                        if (Test-Option2ProtectedAppxPackage -Package $package) {
                            $message = "Protected Windows SystemApp preserved: {0} matched by {1}" -f $package.PackageFullName, $pattern
                            Write-Option2Log $message 'SKIP'
                            $results.PreservedItems.Add($message) | Out-Null
                            $results.SkippedProtectedItems.Add($package.PackageFullName) | Out-Null
                            continue
                        }

                        if (Remove-Option2InstalledAppxPackage -Package $package -Pattern $pattern) {
                            $results.InstalledPackagesRemoved++
                            $results.RemovedItems.Add($package.PackageFullName) | Out-Null
                        }
                        else {
                            $results.FailedRemovals++
                            $results.FailedItems.Add($package.PackageFullName) | Out-Null
                        }
                    }
                }

                if (-not $SkipProvisionedRemoval) {
                    $provisionedPackages = @(Get-Option2ProvisionedPackagesByPattern -Pattern $pattern)
                    foreach ($package in $provisionedPackages) {
                        if (Remove-Option2ProvisionedAppxPackage -Package $package -Pattern $pattern) {
                            $results.ProvisionedPackagesRemoved++
                            $results.RemovedItems.Add($package.PackageName) | Out-Null
                        }
                        else {
                            $results.FailedRemovals++
                            $results.FailedItems.Add($package.PackageName) | Out-Null
                        }
                    }
                }
                else {
                    Write-Option2Log ("Provisioned package removal skipped for pattern: {0}" -f $pattern) 'SKIP'
                }
            }
        }

        Write-Progress -Activity 'Option 2 - Removing Windows bloatware' -Completed

        try {
            Write-Option2Log 'Refreshing machine and user policy.' 'INFO'
            gpupdate.exe /target:computer /force | Out-Null
            gpupdate.exe /target:user /force | Out-Null
            Write-Option2Log 'Policy refresh requested successfully.' 'OK'
        }
        catch {
            Write-Option2Log ("Policy refresh failed or was unavailable: {0}" -f $_.Exception.Message) 'WARN'
        }

        Write-Host ''
        Write-Host ('=' * 72) -ForegroundColor Cyan
        Write-Host 'OPTION 2 BLOATWARE / COPILOT REMOVAL SUMMARY' -ForegroundColor Cyan
        Write-Host ('=' * 72) -ForegroundColor Cyan
        Write-Host ("Patterns processed             : {0}" -f $results.PatternsProcessed) -ForegroundColor White
        Write-Host ("Installed packages removed     : {0}" -f $results.InstalledPackagesRemoved) -ForegroundColor Green
        Write-Host ("Provisioned packages removed   : {0}" -f $results.ProvisionedPackagesRemoved) -ForegroundColor Green
        Write-Host ("Installed patterns not found   : {0}" -f $results.InstalledPackagesNotFound) -ForegroundColor Yellow
        Write-Host ("Protected packages skipped     : {0}" -f $results.SkippedProtectedItems.Count) -ForegroundColor DarkGray
        Write-Host ("Failed removal attempts        : {0}" -f $results.FailedRemovals) -ForegroundColor $(if ($results.FailedRemovals -gt 0) { 'Red' } else { 'Green' })
        Write-Host ("Copilot policies applied       : {0}" -f $results.CopilotPoliciesApplied) -ForegroundColor Green
        Write-Host ("Log file                       : {0}" -f $logPath) -ForegroundColor Cyan
        Write-Host ('=' * 72) -ForegroundColor Cyan

        if ($results.FailedRemovals -gt 0) {
            $global:LastStatus = "[WARN] Option 2 completed with $($results.FailedRemovals) failed removal attempt(s). See $logPath"
        }
        else {
            $global:LastStatus = "[OK] Option 2 completed. Removed $($results.InstalledPackagesRemoved) installed and $($results.ProvisionedPackagesRemoved) provisioned package(s); Copilot disabled."
        }
    }
    catch {
        Write-Option2Log ("Critical error during Option 2: {0}" -f $_.Exception.Message) 'ERROR'
        $global:LastStatus = "[ERROR] Option 2 failed: $($_.Exception.Message)"
    }
    finally {
        try {
            $script:Option2LogEntries | Out-File -LiteralPath $logPath -Encoding UTF8 -Force
            Write-Host "`nDetailed Option 2 log saved to: $logPath" -ForegroundColor Cyan
        }
        catch {
            Write-Host ("Failed to write Option 2 log: {0}" -f $_.Exception.Message) -ForegroundColor Yellow
        }
    }
...
function Set-NTPConfiguration {
    ...
    .DESCRIPTION
    Uses w32tm.exe with explicit argument arrays, input validation, and retry-aware
    verification so configuration failures are reported clearly instead of being
    hidden behind later service start or resync failures.
    #>
    ...
    try {
        ...
        if ($cleanServers.Count -eq 0) {
            throw 'No valid NTP servers were provided.'
        }

        if ($WhatIfPreference) {
            Write-Host "   Would configure NTP servers: $($cleanServers -join ', ')" -ForegroundColor Yellow
            $result.Success = $true
            $result.ConfiguredServers = $cleanServers
            return $result
        }

        $ntpServerList = $cleanServers -join ' '

        if ($DomainEnvironment) {
            Write-StatusLog '[INFO] Configuring for domain time hierarchy' -Level 'Info'
            $syncFlags = 'DOMHIER'
            $result.ConfigurationMethod = 'Domain hierarchy with fallback servers recorded'
        }
        else {
        ...
        $arguments = @(
            '/config'
            "/manualpeerlist:$ntpServerList"
            "/syncfromflags:$syncFlags"
            '/reliable:no'
            '/update'
        )

        $configOutput = & w32tm.exe @arguments 2>&1
        $configExitCode = $LASTEXITCODE
        $result.ExitCode = $configExitCode
        $result.Output = @($configOutput)

        if ($configExitCode -ne 0) {
            $outputText = (@($configOutput) -join ' ').Trim()
            if ([string]::IsNullOrWhiteSpace($outputText)) { $outputText = 'No output returned.' }
            throw "w32tm.exe /config failed with exit code $configExitCode. Output: $outputText"
        }

        Start-Sleep -Seconds 2

        $verified = $false
        $lastVerifyExitCode = $null
        for ($i = 1; $i -le 3; $i++) {
            $configurationOutput = & w32tm.exe /query /configuration 2>&1
            $lastVerifyExitCode = $LASTEXITCODE

            if ($lastVerifyExitCode -eq 0) {
                $configurationText = @($configurationOutput) -join [Environment]::NewLine
                if ($DomainEnvironment -or $configurationText -match [regex]::Escape($cleanServers[0])) {
                    $verified = $true
                    break
                }
            }
        ...
        if (-not $verified) {
            throw "w32tm.exe configuration verification failed after 3 attempts. Last exit code: $lastVerifyExitCode"
        }

        $result.Success = $true
        $result.ConfiguredServers = $cleanServers
        Write-StatusLog "[OK] NTP servers configured: $($cleanServers -join ', ')" -Level 'Success'
    }
    catch {
...
function Stop-W32TimeServiceBounded {
    <#
    .SYNOPSIS
    Stops W32Time with a bounded wait and guarded forced process termination fallback.

    .DESCRIPTION
    sc.exe stop is used instead of Stop-Service so the script does not block forever.
    If the service remains stuck past the timeout, the function rechecks the current
    service PID before terminating it. This avoids killing a stale PID that may have
    ...
    try {
        ...
        if ($service.Status -eq 'Stopped') {
            $result.Success = $true
            Write-StatusLog '[INFO] W32Time service is already stopped' -Level 'Info'
            return $result
        }

        $result.WasRunning = $true
        Write-StatusLog "[INFO] Stopping W32Time service with a $TimeoutSeconds second timeout..." -Level 'Info'

        $serviceInfo = Get-CimInstance -ClassName 'Win32_Service' -Filter "Name='W32Time'" -ErrorAction SilentlyContinue
        if ($serviceInfo -and $serviceInfo.ProcessId -gt 0) {
            $result.ProcessId = [int]$serviceInfo.ProcessId
        }

        $stopOutput = & sc.exe stop W32Time 2>&1
        $stopExitCode = $LASTEXITCODE
        if ($stopExitCode -ne 0 -and $stopExitCode -ne 1062) {
            $stopText = (@($stopOutput) -join ' ').Trim()
            Write-StatusLog "[WARN] sc.exe stop returned exit code $stopExitCode. Output: $stopText" -Level 'Warning'
        }
        ...
        do {
            ...
            if ($service.Status -eq 'Stopped') {
                $result.Success = $true
                Write-StatusLog '[OK] W32Time service stopped cleanly' -Level 'Success'
                return $result
            }
        } while ((Get-Date) -lt $deadline)

        Write-StatusLog "[WARN] W32Time did not stop within $TimeoutSeconds seconds. Checking current service PID before forced termination..." -Level 'Warning'

        $currentServiceInfo = Get-CimInstance -ClassName 'Win32_Service' -Filter "Name='W32Time'" -ErrorAction SilentlyContinue
        if (-not $currentServiceInfo) {
            throw 'W32Time service could not be queried after the stop timeout.'
        }

        if ($currentServiceInfo.State -eq 'Stopped') {
            $result.Success = $true
            Write-StatusLog '[OK] W32Time reached Stopped state after timeout recheck' -Level 'Success'
            return $result
        }

        $currentPid = [int]$currentServiceInfo.ProcessId
        $result.ProcessId = $currentPid

        if ($currentPid -le 4) {
            throw "W32Time is stuck but the service PID is invalid or protected: $currentPid"
        }

        $samePidServices = @(
            Get-CimInstance -ClassName 'Win32_Service' -Filter "ProcessId=$currentPid" -ErrorAction SilentlyContinue |
                Where-Object { $_.State -eq 'Running' } |
                Select-Object -ExpandProperty Name
        )
        $result.SharedProcessServices = $samePidServices

        if ($samePidServices.Count -gt 1) {
            Write-StatusLog "[WARN] PID $currentPid hosts multiple services: $($samePidServices -join ', '). Terminating it may restart/impact those services." -Level 'Warning'
        }

        Write-StatusLog "[WARN] Force-killing W32Time service process PID $currentPid" -Level 'Warning'
        $killOutput = & taskkill.exe /PID $currentPid /F /T 2>&1
        $killExitCode = $LASTEXITCODE
        $result.ForcedKillUsed = $true

        if ($killExitCode -ne 0) {
            $killText = (@($killOutput) -join ' ').Trim()
            throw "taskkill.exe failed with exit code $killExitCode. Output: $killText"
        }

        $killDeadline = (Get-Date).AddSeconds($PostKillWaitSeconds)
        do {
            Start-Sleep -Milliseconds 500
            $service = Get-Service -Name 'W32Time' -ErrorAction Stop
            if ($service.Status -eq 'Stopped') {
                $result.Success = $true
                Write-StatusLog '[OK] W32Time service process was terminated and service is now stopped' -Level 'Success'
                return $result
            }
        } while ((Get-Date) -lt $killDeadline)

        throw "W32Time service still reports status '$($service.Status)' after forced process termination."
    }
    catch {
        $result.Error = "Failed while stopping W32Time service: $($_.Exception.Message)"
        Write-StatusLog "[ERROR] $($result.Error)" -Level 'Error'
    }
...
function Start-W32TimeServiceBounded {
    <#
    .SYNOPSIS
    Starts W32Time and verifies it reaches Running state within a bounded wait.
    #>
    ...
    try {
        ...
        if ($service.Status -eq 'Running') {
            $result.Success = $true
            Write-StatusLog '[INFO] W32Time service is already running' -Level 'Info'
            return $result
        }

        if ($service.Status -eq 'StopPending') {
            Write-StatusLog '[WARN] W32Time is still StopPending before start. Waiting briefly before starting...' -Level 'Warning'
            $deadline = (Get-Date).AddSeconds([Math]::Min($TimeoutSeconds, 10))
            do {
                Start-Sleep -Milliseconds 500
                $service.Refresh()
            } while ($service.Status -eq 'StopPending' -and (Get-Date) -lt $deadline)
        }

        Write-StatusLog '[INFO] Starting W32Time service...' -Level 'Info'
        ...
        do {
            ...
            if ($service.Status -eq 'Running') {
                $result.Success = $true
                Write-StatusLog '[OK] W32Time service is running' -Level 'Success'
                return $result
            }
            Start-Sleep -Milliseconds 500
        } while ((Get-Date) -lt $deadline)

        throw "W32Time did not reach Running state within $TimeoutSeconds seconds. Current status: $($service.Status)"
    }
    catch {
        $result.Error = "Failed to start W32Time service: $($_.Exception.Message)"
        Write-StatusLog "[ERROR] $($result.Error)" -Level 'Error'
    }
...
function Set-W32TimeServiceConfiguration {
    <#
    .SYNOPSIS
    Configures and restarts the Windows Time service using bounded waits.
    #>
    ...
    try {
        if ($WhatIfPreference) {
            Write-Host '   Would configure W32Time service (Automatic startup, bounded stop, guarded forced kill if stuck, bounded start)' -ForegroundColor Yellow
            $result.Success = $true
            return $result
        }
        ...
        $stopResult = Stop-W32TimeServiceBounded -TimeoutSeconds 10 -PostKillWaitSeconds 5
        $result.ForcedStopUsed = [bool]$stopResult.ForcedKillUsed
        $result.SharedProcessServices = @($stopResult.SharedProcessServices)
        ...
        $startResult = Start-W32TimeServiceBounded -TimeoutSeconds 20
        ...
        if ($result.ForcedStopUsed) {
            Write-StatusLog '[OK] W32Time service restarted after forced stop fallback' -Level 'Success'
        }
        else {
            Write-StatusLog '[OK] W32Time service restarted cleanly to apply configuration' -Level 'Success'
        }
        ...
        for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
            ...
            if ($syncExitCode -eq 0) {
                ...
                if ($statusExitCode -eq 0) {
                    $syncSourceLine = @($status) | Where-Object { $_ -match '^\s*Source\s*:' } | Select-Object -First 1
                    if ($syncSourceLine) {
                        $result.SyncSource = (($syncSourceLine -split ':', 2)[1]).Trim()
                    }
            ...
            $outputText = (@($syncOutput) -join ' ').Trim()
            if ([string]::IsNullOrWhiteSpace($outputText)) { $outputText = 'No output returned.' }
            Write-StatusLog "[WARN] Time sync attempt $attempt failed with exit code $syncExitCode. Output: $outputText" -Level 'Warning'

            if ($attempt -lt $MaxAttempts) {
                Start-Sleep -Seconds (2 * $attempt)
                $service = Get-Service -Name 'W32Time' -ErrorAction SilentlyContinue
                if ($service -and $service.Status -ne 'Running') {
                    $null = Start-W32TimeServiceBounded -TimeoutSeconds 20
                }
            }
        }

        $result.TimeAfter = Get-Date
        $lastText = (@($result.LastOutput) -join ' ').Trim()
        if ([string]::IsNullOrWhiteSpace($lastText)) { $lastText = 'No output returned.' }
        $result.Error = "Time sync failed after $MaxAttempts attempts. Last output: $lastText"
...
function Run-DiskCleanup {
    <#
    .SYNOPSIS
    Runs an SSD-safe disk cleanup and storage health check.

    .DESCRIPTION
    Cleans common Windows temporary/cache locations, empties the Recycle Bin,
    clears Windows Update and Delivery Optimization download caches, runs DISM
    component cleanup without ResetBase, checks SSD/NVMe wear with
    Get-StorageReliabilityCounter, and alerts the technician when reported wear
    is above the configured threshold.

    This function intentionally does not run traditional defrag against SSD or
    NVMe drives and does not perform secure wiping/free-space wiping because
    those actions add unnecessary writes and can reduce SSD life.
    #>
    [CmdletBinding(SupportsShouldProcess)]
    param(
    ...
    $script:DiskCleanupLogEntries = New-Object System.Collections.Generic.List[string]
    $script:DiskCleanupSummary = [ordered]@{
        DeletedItems       = 0
        FailedItems        = 0
        LockedItems        = 0
        AccessDeniedItems  = 0
    ...
    function Write-DiskCleanupLog {
        ...
        [void]$script:DiskCleanupLogEntries.Add($line)
    ...
    function Test-DiskCleanupAdministrator {
        try {
            $identity = [Security.Principal.WindowsIdentity]::GetCurrent()
            $principal = New-Object Security.Principal.WindowsPrincipal($identity)
            return $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
    ...
    function Show-DiskCleanupTechWarning {
        param(
            [Parameter(Mandatory)][string]$Title,
            [Parameter(Mandatory)][string]$Message
        )

        Write-DiskCleanupLog $Message 'WARN'
        ...
        try {
            ...
            Write-DiskCleanupLog ('Windows Forms popup was unavailable: {0}' -f $_.Exception.Message) 'SKIP'
        }

        try {
            $msgExe = Join-Path $env:SystemRoot 'System32\msg.exe'
            if (Test-Path -LiteralPath $msgExe) {
                & $msgExe * /TIME:90 $Message 2>$null | Out-Null
            }
        }
        catch {
            Write-DiskCleanupLog ('msg.exe notification failed: {0}' -f $_.Exception.Message) 'SKIP'
        }
    }
    ...
    function Get-FileLockOwnerHint {
        <#
        .SYNOPSIS
        Best-effort reporting of processes that may be using a file.

        .DESCRIPTION
        Uses the Windows Restart Manager API to identify processes registered
        against a file path. This is report-only. It intentionally does not stop
        or kill any process because killing a process that owns a temp/cache file
        can corrupt active application state or destabilize Windows services.
        #>
        param([Parameter(Mandatory)][string]$Path)

        if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
            return 'Lock owner check skipped because the item is not a file.'
        }

        try {
            if (-not ('DiskCleanup.RestartManager' -as [type])) {
...
'@
            ...
            $startResult = [DiskCleanup.RestartManager]::RmStartSession([ref]$sessionHandle, 0, $sessionKey)
            if ($startResult -ne 0) {
                return ('Restart Manager unavailable. RmStartSession returned {0}.' -f $startResult)
            }

            try {
                $files = [string[]]@($Path)
                $registerResult = [DiskCleanup.RestartManager]::RmRegisterResources($sessionHandle, [uint32]1, $files, [uint32]0, [IntPtr]::Zero, [uint32]0, $null)
                if ($registerResult -ne 0) {
                    return ('Restart Manager could not register file. RmRegisterResources returned {0}.' -f $registerResult)
                }

                $needed = [uint32]0
                $count = [uint32]0
                $rebootReasons = [uint32]0
                [void][DiskCleanup.RestartManager]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $null, [ref]$rebootReasons)

                if ($needed -eq 0) {
                    return 'No owning process reported by Restart Manager.'
                }

                $count = $needed
                $processInfo = New-Object 'DiskCleanup.RestartManager+RM_PROCESS_INFO[]' $count
                $listResult = [DiskCleanup.RestartManager]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $processInfo, [ref]$rebootReasons)
                if ($listResult -ne 0) {
                    return ('Restart Manager could not list locking processes. RmGetList returned {0}.' -f $listResult)
                }
                ...
                for ($i = 0; $i -lt [int]$count; $i++) {
                    ...
                    $serviceName = $processInfo[$i].strServiceShortName
                    $appType = $processInfo[$i].ApplicationType.ToString()
                    if ([string]::IsNullOrWhiteSpace($appName)) { $appName = 'Unknown' }
                    if (-not [string]::IsNullOrWhiteSpace($serviceName)) {
                        [void]$owners.Add(('{0} PID {1}, service {2}, type {3}' -f $appName, $pid, $serviceName, $appType))
                    }
                    else {
                        [void]$owners.Add(('{0} PID {1}, type {2}' -f $appName, $pid, $appType))
                    }
                }
                ...
                if ($sessionHandle -ne 0) {
                    [void][DiskCleanup.RestartManager]::RmEndSession($sessionHandle)
                }
            }
        }
        catch {
            return ('Unable to determine lock owner safely: {0}' -f $_.Exception.Message)
        }
    }
    ...
    function Remove-DirectoryContentsSafe {
        ...
        if (-not (Test-Path -LiteralPath $Path)) {
            Write-DiskCleanupLog ('Path not found, skipping {0}: {1}' -f $Description, $Path) 'SKIP'
            $script:DiskCleanupSummary.SkippedItems++
            return
        }

        Write-DiskCleanupLog ('Cleaning {0}: {1}' -f $Description, $Path) 'INFO'

        $cutoff = $null
        if ($OlderThanDays -ge 0) {
            $cutoff = (Get-Date).AddDays(-[int]$OlderThanDays)
            Write-DiskCleanupLog ('Only removing items older than {0} day(s).' -f $OlderThanDays) 'INFO'
        }
        ...
        try {
            ...
            Write-DiskCleanupLog ('Failed to enumerate {0}: {1}' -f $Path, $_.Exception.Message) 'WARN'
            $script:DiskCleanupSummary.FailedItems++
            return
        }

        foreach ($item in $items) {
            try {
                if ($null -ne $cutoff -and $item.LastWriteTime -gt $cutoff) {
                    $script:DiskCleanupSummary.SkippedItems++
                    continue
                }

                if ($PSCmdlet.ShouldProcess($item.FullName, 'Remove temporary/cache item')) {
                    Remove-Item -LiteralPath $item.FullName -Recurse -Force -ErrorAction Stop
                    $script:DiskCleanupSummary.DeletedItems++
                }
            }
            catch {
                $exceptionMessage = $_.Exception.Message
                $isLockedOrInUse = ($exceptionMessage -match 'being used by another process|because it is being used|process cannot access|used by another process|The requested operation cannot be performed on a file with a user-mapped section open')
                $isAccessDenied = ($exceptionMessage -match 'Access to the path .* is denied|UnauthorizedAccessException|access is denied')

                if ($isLockedOrInUse) {
                    $script:DiskCleanupSummary.LockedItems++
                    $script:DiskCleanupSummary.SkippedItems++
                    $ownerHint = Get-FileLockOwnerHint -Path $item.FullName
                    Write-DiskCleanupLog ('Skipped locked/in-use item: {0}' -f $item.FullName) 'SKIP'
                    Write-DiskCleanupLog ('Lock owner hint: {0}' -f $ownerHint) 'INFO'
                }
                elseif ($isAccessDenied) {
                    $script:DiskCleanupSummary.AccessDeniedItems++
                    $script:DiskCleanupSummary.SkippedItems++
                    Write-DiskCleanupLog ('Skipped access-denied item: {0} ({1})' -f $item.FullName, $exceptionMessage) 'SKIP'
                }
                else {
                    Write-DiskCleanupLog ('Failed to remove {0}: {1}' -f $item.FullName, $exceptionMessage) 'WARN'
                    $script:DiskCleanupSummary.FailedItems++
                }
            }
        }

        $script:DiskCleanupSummary.CleanupActions++
    }

    function Invoke-ProcessWithTimeoutSafe {
        <#
        .SYNOPSIS
        Runs an external process with timeout, periodic status updates, and log capture.

        .DESCRIPTION
        DISM StartComponentCleanup can legitimately run for a long time and may appear
        frozen if launched with a simple WaitForExit call. This helper starts the process,
        writes a status line at a regular interval, tracks elapsed time, enforces a timeout,
        and captures stdout/stderr to temporary files so redirected output cannot deadlock
        the process.
        #>
        param(
        ...
        try {
            if (-not (Test-Path -LiteralPath $FilePath)) {
                Write-DiskCleanupLog ('Process not found for {0}: {1}' -f $Description, $FilePath) 'WARN'
                return $false
            }
            ...
            while (-not $process.HasExited) {
                ...
                if ($elapsedSeconds -ge $TimeoutSeconds) {
                    Write-DiskCleanupLog ('{0} exceeded timeout after {1}. Attempting to stop process ID {2}.' -f $Description, $stopwatch.Elapsed.ToString('hh\:mm\:ss'), $process.Id) 'WARN'
                    try {
                        Stop-Process -Id $process.Id -Force -ErrorAction Stop
                    }
                    catch {
                        Write-DiskCleanupLog ('Failed to stop timed-out process ID {0}: {1}' -f $process.Id, $_.Exception.Message) 'WARN'
                    }
                    return $false
                }

                if ($elapsedSeconds -gt 0 -and ($elapsedSeconds % $StatusIntervalSeconds -eq 0) -and $elapsedSeconds -ne $lastStatusSeconds) {
                    $lastStatusSeconds = $elapsedSeconds
                    Write-DiskCleanupLog ('Still running {0}. Elapsed: {1}. Process ID: {2}.' -f $Description, $stopwatch.Elapsed.ToString('hh\:mm\:ss'), $process.Id) 'INFO'
                }
            }
            ...
            if ($stdoutPath -and (Test-Path -LiteralPath $stdoutPath)) {
                ...
                if (-not [string]::IsNullOrWhiteSpace($stdout)) {
                    $stdout.Trim() -split "`r?`n" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | ForEach-Object {
                        Write-DiskCleanupLog ([string]$_) 'INFO'
                    }
                }
            }
            ...
            if ($stderrPath -and (Test-Path -LiteralPath $stderrPath)) {
                ...
                if (-not [string]::IsNullOrWhiteSpace($stderr)) {
                    $stderr.Trim() -split "`r?`n" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | ForEach-Object {
                        Write-DiskCleanupLog ([string]$_) 'WARN'
                    }
                }
            }

            if ($process.ExitCode -eq 0) {
                Write-DiskCleanupLog ('Completed {0} successfully. Duration: {1}.' -f $Description, $stopwatch.Elapsed.ToString('hh\:mm\:ss')) 'OK'
                return $true
            }

            Write-DiskCleanupLog ('{0} exited with code {1}. Duration: {2}.' -f $Description, $process.ExitCode, $stopwatch.Elapsed.ToString('hh\:mm\:ss')) 'WARN'
            return $false
        }
        catch {
            Write-DiskCleanupLog ('Failed running {0}: {1}' -f $Description, $_.Exception.Message) 'WARN'
            return $false
        }
        finally {
    ...
    function Stop-ServiceForCleanupSafe {
        param(
            [Parameter(Mandatory)][string]$Name,
            [int]$TimeoutSeconds = 15
        )
        ...
        try {
            ...
            if ($service.Status -eq 'Stopped') {
                Write-DiskCleanupLog ('Service already stopped: {0}' -f $Name) 'OK'
                return $true
            }

            Write-DiskCleanupLog ('Stopping service for cleanup: {0}' -f $Name) 'INFO'
            Stop-Service -Name $Name -Force -ErrorAction SilentlyContinue
            $service.WaitForStatus('Stopped', [TimeSpan]::FromSeconds($TimeoutSeconds))
            Write-DiskCleanupLog ('Stopped service: {0}' -f $Name) 'OK'
            return $true
        }
        catch {
            Write-DiskCleanupLog ('Could not stop service {0}: {1}' -f $Name, $_.Exception.Message) 'WARN'
            return $false
        }
    }

    function Start-ServiceForCleanupSafe {
        ...
        try {
            ...
            if ($service.Status -ne 'Running') {
                Start-Service -Name $Name -ErrorAction Stop
                Write-DiskCleanupLog ('Restarted service: {0}' -f $Name) 'OK'
            }
        }
        catch {
            Write-DiskCleanupLog ('Could not restart service {0}: {1}' -f $Name, $_.Exception.Message) 'WARN'
        }
    }

    function Get-SSDHealthSummary {
        [CmdletBinding()]
        param([int]$WearWarningPercent = 10)
        ...
        try {
            ...
            Write-DiskCleanupLog ('Unable to query physical disks: {0}' -f $_.Exception.Message) 'WARN'
            return $results
        }
        ...
        foreach ($physicalDisk in $physicalDisks) {
            ...
            $isSolidState = ($mediaType -match 'SSD') -or ($busType -match 'NVMe')
            ...
            try {
                ...
                Write-DiskCleanupLog ('Storage reliability counters unavailable for {0}: {1}' -f $physicalDisk.FriendlyName, $_.Exception.Message) 'WARN'
            }
            ...
            $wearText = if ($null -ne $wear) { ('{0}%' -f $wear) } else { 'Not reported by device/driver' }
            Write-DiskCleanupLog ('SSD/NVMe health: {0} | Media={1} | Bus={2} | Health={3} | Wear={4}' -f $entry.FriendlyName, $entry.MediaType, $entry.BusType, $entry.HealthStatus, $wearText) 'INFO'

            if ($null -ne $wear -and [int]$wear -gt $WearWarningPercent) {
                $script:DiskCleanupSummary.WearWarnings++
                Show-DiskCleanupTechWarning -Title 'SSD/NVMe Wear Warning' -Message ('SSD/NVMe wear is above {0}% on {1}. Reported wear: {2}%. HealthStatus: {3}. Recommend monitoring/replacement planning.' -f $WearWarningPercent, $entry.FriendlyName, $wear, $entry.HealthStatus)
            }
        }

        if ($results.Count -eq 0) {
            Write-DiskCleanupLog 'No SSD/NVMe disks with available storage metadata were detected.' 'WARN'
        }

        return $results
    }

    function Invoke-SSDReTrimSafe {
        [CmdletBinding(SupportsShouldProcess)]
        param()

        try {
            $disks = @(Get-Disk -ErrorAction Stop | Where-Object { $_.BusType -eq 'NVMe' -or $_.MediaType -eq 'SSD' })
        }
        catch {
            Write-DiskCleanupLog ('Unable to enumerate disks for SSD ReTrim: {0}' -f $_.Exception.Message) 'WARN'
            return
        }
        ...
        foreach ($disk in $disks) {
            try {
                ...
                foreach ($partition in $partitions) {
                    ...
                    if ($PSCmdlet.ShouldProcess(('{0}:' -f $driveLetter), 'Run SSD ReTrim only')) {
                        Write-DiskCleanupLog ('Running SSD-safe ReTrim on {0}: drive. No defrag will be run.' -f $driveLetter) 'INFO'
                        try {
                            Optimize-Volume -DriveLetter $driveLetter -ReTrim -Verbose -ErrorAction Stop 4>&1 | ForEach-Object {
                                Write-DiskCleanupLog ([string]$_) 'INFO'
                            }
                            $script:DiskCleanupSummary.ReTrimActions++
                            Write-DiskCleanupLog ('Completed ReTrim on {0}: drive.' -f $driveLetter) 'OK'
                        }
                        catch {
                            Write-DiskCleanupLog ('ReTrim failed on {0}: drive: {1}' -f $driveLetter, $_.Exception.Message) 'WARN'
                        }
                    }
                }
            }
            catch {
                Write-DiskCleanupLog ('Failed processing disk {0}: {1}' -f $disk.Number, $_.Exception.Message) 'WARN'
            }
        }
    }

    function Clear-WindowsUpdateDownloadCacheSafe {
        [CmdletBinding(SupportsShouldProcess)]
        param()

        $services = @('UsoSvc','wuauserv','bits')
        foreach ($serviceName in $services) {
            Stop-ServiceForCleanupSafe -Name $serviceName -TimeoutSeconds 15 | Out-Null
        }

        try {
            Remove-DirectoryContentsSafe -Path (Join-Path $env:SystemRoot 'SoftwareDistribution\Download') -Description 'Windows Update download cache'
        }
        finally {
            foreach ($serviceName in @('bits','wuauserv','UsoSvc')) {
                Start-ServiceForCleanupSafe -Name $serviceName
            }
        }
    }

    try {
        Write-DiskCleanupLog '=== Option 12 SSD-Safe Disk Cleanup Started ===' 'INFO'

        if (-not (Test-DiskCleanupAdministrator)) {
        ...
        $freeBefore = $null
        try {
            $systemVolume = Get-CimInstance -ClassName Win32_LogicalDisk -Filter ("DeviceID='{0}'" -f $systemDrive) -ErrorAction Stop
            $freeBefore = [int64]$systemVolume.FreeSpace
            Write-DiskCleanupLog ('Free space before cleanup on {0}: {1}' -f $systemDrive, (Format-BytesHumanReadable $freeBefore)) 'INFO'
        }
        catch {
            Write-DiskCleanupLog ('Unable to read starting free space: {0}' -f $_.Exception.Message) 'WARN'
        }

        $healthResults = Get-SSDHealthSummary -WearWarningPercent $WearWarningPercent

        if (-not $SkipTempFolders) {
            $cleanupTargets = @(
                @{ Path = $env:TEMP; Description = 'Current user temporary files'; OlderThanDays = 1 },
                @{ Path = (Join-Path $env:SystemRoot 'Temp'); Description = 'Windows temporary files'; OlderThanDays = 1 },
                @{ Path = (Join-Path $env:SystemRoot 'Prefetch'); Description = 'Windows Prefetch files older than 30 days'; OlderThanDays = 30 },
                @{ Path = (Join-Path $env:ProgramData 'Microsoft\Windows\WER\ReportArchive'); Description = 'Archived Windows Error Reporting data'; OlderThanDays = 7 },
                @{ Path = (Join-Path $env:ProgramData 'Microsoft\Windows\WER\ReportQueue'); Description = 'Queued Windows Error Reporting data'; OlderThanDays = 7 },
                @{ Path = (Join-Path $env:ProgramData 'Microsoft\Windows\DeliveryOptimization\Cache'); Description = 'Delivery Optimization cache'; OlderThanDays = 1 },
                @{ Path = (Join-Path $env:SystemRoot 'ServiceProfiles\NetworkService\AppData\Local\Microsoft\Windows\DeliveryOptimization\Cache'); Description = 'Delivery Optimization service cache'; OlderThanDays = 1 },
                @{ Path = (Join-Path $env:ProgramData 'Microsoft\Windows\Caches'); Description = 'Windows ProgramData cache files'; OlderThanDays = 7 },
                @{ Path = (Join-Path $env:LocalAppData 'Microsoft\Windows\INetCache'); Description = 'Current user INet cache'; OlderThanDays = 1 },
                @{ Path = (Join-Path $env:LocalAppData 'Microsoft\Windows\WebCache'); Description = 'Current user WebCache'; OlderThanDays = 7 }
            )

            foreach ($target in $cleanupTargets) {
                Remove-DirectoryContentsSafe -Path $target.Path -Description $target.Description -OlderThanDays $target.OlderThanDays
            }

            # Clean common browser cache folders for all local user profiles without touching passwords, bookmarks, or profile settings.
            ...
            if (Test-Path -LiteralPath $profileRoot) {
                Get-ChildItem -LiteralPath $profileRoot -Directory -Force -ErrorAction SilentlyContinue | Where-Object {
                    ...
                    $browserCacheTargets = @(
                        @{ Path = (Join-Path $userProfile 'AppData\Local\Microsoft\Edge\User Data\Default\Cache'); Description = ('Edge cache for {0}' -f $_.Name); OlderThanDays = 1 },
                        @{ Path = (Join-Path $userProfile 'AppData\Local\Microsoft\Edge\User Data\Default\Code Cache'); Description = ('Edge code cache for {0}' -f $_.Name); OlderThanDays = 1 },
                        @{ Path = (Join-Path $userProfile 'AppData\Local\Google\Chrome\User Data\Default\Cache'); Description = ('Chrome cache for {0}' -f $_.Name); OlderThanDays = 1 },
                        @{ Path = (Join-Path $userProfile 'AppData\Local\Google\Chrome\User Data\Default\Code Cache'); Description = ('Chrome code cache for {0}' -f $_.Name); OlderThanDays = 1 },
                        @{ Path = (Join-Path $userProfile 'AppData\Local\Mozilla\Firefox\Profiles'); Description = ('Firefox profile caches for {0}' -f $_.Name); OlderThanDays = -1 }
                    )

                    foreach ($cacheTarget in $browserCacheTargets) {
                        if ($cacheTarget.Description -like 'Firefox*' -and (Test-Path -LiteralPath $cacheTarget.Path)) {
                            Get-ChildItem -LiteralPath $cacheTarget.Path -Directory -ErrorAction SilentlyContinue | ForEach-Object {
                                Remove-DirectoryContentsSafe -Path (Join-Path $_.FullName 'cache2\entries') -Description ('Firefox cache for {0}' -f $_.Name) -OlderThanDays 1
                                Remove-DirectoryContentsSafe -Path (Join-Path $_.FullName 'startupCache') -Description ('Firefox startup cache for {0}' -f $_.Name) -OlderThanDays 7
                            }
                        }
                        else {
                            Remove-DirectoryContentsSafe -Path $cacheTarget.Path -Description $cacheTarget.Description -OlderThanDays $cacheTarget.OlderThanDays
                        }
                    }
                }
            }
            ...
            Write-DiskCleanupLog 'Temporary folder cleanup skipped by parameter.' 'SKIP'
        }

        if (-not $SkipSystemCleanup) {
            Clear-WindowsUpdateDownloadCacheSafe

            try {
                Write-DiskCleanupLog 'Clearing Recycle Bin for all drives.' 'INFO'
                if ($PSCmdlet.ShouldProcess('Recycle Bin', 'Clear all recycle bins')) {
                    Clear-RecycleBin -Force -ErrorAction Stop
                    Write-DiskCleanupLog 'Recycle Bin cleared.' 'OK'
                    $script:DiskCleanupSummary.CleanupActions++
                }
            }
            catch {
                Write-DiskCleanupLog ('Recycle Bin cleanup failed or was not applicable: {0}' -f $_.Exception.Message) 'WARN'
            }

            $dismExe = Join-Path $env:SystemRoot 'System32\dism.exe'
            if (Test-Path -LiteralPath $dismExe) {
                [void](Invoke-ProcessWithTimeoutSafe -FilePath $dismExe -ArgumentList @('/Online','/Cleanup-Image','/StartComponentCleanup') -TimeoutSeconds $TimeoutSeconds -Description 'DISM component store cleanup')
            }
            else {
                Write-DiskCleanupLog 'DISM executable was not found; component cleanup skipped.' 'WARN'
            }

            # Remove old CBS compressed logs only; do not delete active CBS.log.
            Remove-DirectoryContentsSafe -Path (Join-Path $env:SystemRoot 'Logs\CBS') -Description 'Old CBS compressed/log files' -OlderThanDays 14
        }
        else {
            Write-DiskCleanupLog 'System cleanup skipped by parameter.' 'SKIP'
        }

        if (-not $SkipSSDTrim) {
            Invoke-SSDReTrimSafe
        }
        else {
            Write-DiskCleanupLog 'SSD ReTrim skipped by parameter.' 'SKIP'
        }

        $freeAfter = $null
        try {
            $systemVolumeAfter = Get-CimInstance -ClassName Win32_LogicalDisk -Filter ("DeviceID='{0}'" -f $systemDrive) -ErrorAction Stop
            $freeAfter = [int64]$systemVolumeAfter.FreeSpace
            Write-DiskCleanupLog ('Free space after cleanup on {0}: {1}' -f $systemDrive, (Format-BytesHumanReadable $freeAfter)) 'INFO'

            if ($null -ne $freeBefore) {
                $recovered = $freeAfter - $freeBefore
                Write-DiskCleanupLog ('Estimated space recovered: {0}' -f (Format-BytesHumanReadable $recovered)) 'OK'
            }
        }
        catch {
            Write-DiskCleanupLog ('Unable to read ending free space: {0}' -f $_.Exception.Message) 'WARN'
        }

        $script:DiskCleanupSummary.EndTime = Get-Date
        Write-DiskCleanupLog ('Deleted items: {0}; Failed items: {1}; Locked/in-use skipped: {2}; Access-denied skipped: {3}; Total skipped: {4}; SSD wear warnings: {5}; ReTrim actions: {6}' -f $script:DiskCleanupSummary.DeletedItems, $script:DiskCleanupSummary.FailedItems, $script:DiskCleanupSummary.LockedItems, $script:DiskCleanupSummary.AccessDeniedItems, $script:DiskCleanupSummary.SkippedItems, $script:DiskCleanupSummary.WearWarnings, $script:DiskCleanupSummary.ReTrimActions) 'INFO'

        if ($script:DiskCleanupSummary.WearWarnings -gt 0) {
            $global:LastStatus = '[WARN] Option 12 cleanup completed, but one or more SSD/NVMe drives reported wear above the warning threshold.'
        }
        elseif ($script:DiskCleanupSummary.FailedItems -gt 0) {
            $global:LastStatus = ('[WARN] Option 12 cleanup completed with {0} item-level cleanup warning(s). Review {1}.' -f $script:DiskCleanupSummary.FailedItems, $LogPath)
        }
        else {
            $global:LastStatus = ('[OK] Option 12 SSD-safe disk cleanup completed. Locked/in-use temp files skipped: {0}. Review {1}.' -f $script:DiskCleanupSummary.LockedItems, $LogPath)
        }
    }
    catch {
        Write-DiskCleanupLog ('Critical Option 12 failure: {0}' -f $_.Exception.Message) 'ERROR'
        $global:LastStatus = ('[ERROR] Option 12 disk cleanup failed: {0}' -f $_.Exception.Message)
    }
    finally {
        $ErrorActionPreference = $previousErrorActionPreference
        ...
        try {
            if ($script:DiskCleanupSummary.EndTime -eq $null) {
                $script:DiskCleanupSummary.EndTime = Get-Date
            }
            Write-DiskCleanupLog ('Duration: {0:N2} seconds' -f (($script:DiskCleanupSummary.EndTime) - ($script:DiskCleanupSummary.StartTime)).TotalSeconds) 'INFO'
            Write-DiskCleanupLog '=== Option 12 SSD-Safe Disk Cleanup Completed ===' 'INFO'
            $script:DiskCleanupLogEntries | Out-File -LiteralPath $LogPath -Encoding UTF8 -Force
            Write-Host ''
            Write-Host ('Detailed Option 12 log saved to: {0}' -f $LogPath) -ForegroundColor Cyan
        }
        catch {
            Write-Host ('Failed to write Option 12 log: {0}' -f $_.Exception.Message) -ForegroundColor Yellow
        }
    }
}
...
# Updated from 08_System_Repair.ps1 v3.9 on 2026-05-29
# -----------------------------------------------------------------------------
function Invoke-SystemMaintenance {
    ...
    param(
        ...
        [switch]$AllowHpDriverRepairFromCbs = $true,
    ...
    try {
        ...
        if (-not (Test-IsAdministrator)) {
            Write-Error "This script must be run as Administrator."
            $global:LastStatus = '[ERROR] Option 13 System Repair must be run as Administrator.'
            return
        }
        ...
        Invoke-Safely -Name 'RpcDiagnostics' -ScriptBlock {
            Invoke-RpcDiagnostics
        } -WarnOnly | Out-Null
        ...
        Invoke-Safely -Name 'ExplorerHealthDiagnostics' -ScriptBlock {
            Invoke-ExplorerHealthDiagnostics
        } -WarnOnly | Out-Null
        ...
        if ($AutoRepairOnDetection) {
            ...
            if ($script:Summary.SfcIntegrityViolations) {
                Invoke-Safely -Name 'SfcCbsRepairWorkflow' -ScriptBlock {
                    Invoke-SystemFileRepairWorkflow
                } -WarnOnly | Out-Null
            }

            if ($script:Summary.CbsDriverCorruptionDetected -and $script:Summary.SfcIntegrityViolations) {
                Invoke-Safely -Name 'Tier3HpiaDriverRemediationFromCbs' -ScriptBlock {
                    Invoke-HpiaDriverOnlyRepairFromCbs
                } -WarnOnly | Out-Null
            }
            ...
            if ($script:Summary.DiskCorruptionSuspected) {
                $spotFixSucceeded = $false
                $script:RepairVolumeSpotFixResult = $false
                Invoke-Safely -Name 'RepairVolumeSpotFix' -ScriptBlock {
                    $script:RepairVolumeSpotFixResult = Invoke-RepairVolumeSpotFix
                } -WarnOnly | Out-Null
                $spotFixSucceeded = [bool]$script:RepairVolumeSpotFixResult

                if ($spotFixSucceeded) {
                    $script:RepairVolumePostSpotFixScan = $null
                    Invoke-Safely -Name 'RepairVolumePostSpotFixScan' -ScriptBlock {
                        $script:RepairVolumePostSpotFixScan = Invoke-RepairVolumeScan -StepName 'RepairVolumePostSpotFixScan' -UpdateSummary:$true
                    } -WarnOnly | Out-Null

                    if ($null -ne $script:RepairVolumePostSpotFixScan -and -not [bool]$script:RepairVolumePostSpotFixScan.CorruptionDetected) {
                        Write-Log 'Repair-Volume post-SpotFix verification scan is clean. Offline disk repair will not be scheduled.' 'OK'
                    }
                }
        ...
        if ($AutoRebootIfNeeded -and ($script:Summary.RebootRequired -or $script:Summary.PendingRebootDetected)) {
            try {
                ...
                Show-Summary
                Write-YamlLog
                $global:LastStatus = '[ERROR] Option 13 System Repair completed with failures. Review the YAML log for details.'
                return
            }
        }
        ...
        elseif ($script:Summary.RebootRequired -or $script:Summary.PendingRebootDetected) {
            Write-YamlLog
            $global:LastStatus = '[OK] Option 13 System Repair completed. Reboot is recommended or required. Review the YAML log for details.'
            return
        }
        else {
            Write-YamlLog
            $global:LastStatus = '[OK] Option 13 System Repair completed successfully. Review the YAML log for details.'
            return
        }
    }
    finally {
...
"@
        ...
        Write-Host ("=" * 78) -ForegroundColor Red
        Write-Host "The following local user profiles are selected for PERMANENT deletion:" -ForegroundColor Yellow
        foreach ($profileInfo in $validatedProfiles) {
            $sizeText = if ($profileInfo.Size -gt 0) { " - approx. $([math]::Round($profileInfo.Size, 2)) MB" } else { "" }
            Write-Host ("  - {0} [{1}]{2}" -f $profileInfo.Name, $profileInfo.Path, $sizeText) -ForegroundColor White
        }
        Write-Host ""
        Write-Host ("Profiles to delete : {0}" -f $validatedProfiles.Count) -ForegroundColor Yellow
        Write-Host ("Estimated space    : {0} MB" -f ([math]::Round($totalSize, 2))) -ForegroundColor Yellow
        Write-Host ""
        Write-Host "The following profiles/names are excluded and will NOT be deleted:" -ForegroundColor Cyan
        $ExcludedProfiles | ForEach-Object { Write-Host ("  - {0}" -f $_) -ForegroundColor Green }
        if ($activeUsers.Count -gt 0) {
            Write-Host ""
            Write-Host "Active user sessions were detected and skipped:" -ForegroundColor Yellow
            $activeUsers | Sort-Object -Unique | ForEach-Object { Write-Host ("  - {0}" -f $_) -ForegroundColor Yellow }
        }
        Write-Host ""
        ...
        if (-not $Force) {
            $confirmation = Read-Host "Type Yes in full to permanently delete the listed profiles"
            if ($confirmation -ine 'Yes') {
                Write-LogEntry "Operation cancelled. User did not type Yes." 'WARNING'
                $global:LastStatus = "[WARN] User cancelled profile cleanup before deletion."
                return
            }
            Write-LogEntry "User confirmed profile deletion by typing Yes." 'WARNING'
        }
        else {
            Write-LogEntry "Force switch supplied. Interactive confirmation skipped." 'WARNING'
        }
...
# -----------------------------------------------------------------------------
# Option 15 - Enable Automatic Login with CC-Student
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Option 16 - Set OneDrive to Automatically Login at Boot
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Domain-safe network optimization for managed campus workstations.
# Design choices:
#   - DNS remains dynamic so campus DHCP/domain DNS is used.
#   - IPv6 remains disabled per campus security baseline.
#   - No public DNS servers are configured.
#   - No jumbo frames are enabled because switch-path MTU cannot be assumed.
#   - Adapter features are applied only when supported by the installed NIC driver.

$Script:NetworkOptimizationLogPath = $null
$Script:NetworkOptimizationResults = New-Object System.Collections.Generic.List[object]
...
function Initialize-NetworkOptimizationLogging {
    ...
    $Script:NetworkOptimizationResults.Clear()
}

function Write-NetworkOptimizationLog {
    [CmdletBinding()]
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($Message)) {
        Write-Host ''
        if ($Script:NetworkOptimizationLogPath) { Add-Content -LiteralPath $Script:NetworkOptimizationLogPath -Value '' -ErrorAction SilentlyContinue }
        return
    }
    ...
    switch ($Level) {
        ...
        'SKIP'  { Write-Host $entry -ForegroundColor DarkYellow }
        default { Write-Host $entry -ForegroundColor White }
    }

    if ($Script:NetworkOptimizationLogPath) {
        Add-Content -LiteralPath $Script:NetworkOptimizationLogPath -Value $entry -ErrorAction SilentlyContinue
    }
}

function Add-NetworkOptimizationResult {
    [CmdletBinding()]
    param(
    ...
    $Script:NetworkOptimizationResults.Add([pscustomobject]@{
        Time    = Get-Date
        Action  = $Action
        Success = $Success
        Details = $Details
...
function Test-NetworkOptimizationAdministrator {
    [CmdletBinding()]
    param()
...
function Invoke-NetworkExternalCommand {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$FilePath,
    ...
    try {
        ...
        $exitCode = $LASTEXITCODE
        if ($exitCode -eq 0) {
            Write-NetworkOptimizationLog "$Description completed successfully." 'OK'
            Add-NetworkOptimizationResult -Action $Description -Success $true -Details (($output | Out-String).Trim())
            return $true
        }

        $details = (($output | Out-String).Trim())
        Write-NetworkOptimizationLog "$Description failed with exit code $exitCode. $details" 'WARN'
        Add-NetworkOptimizationResult -Action $Description -Success $false -Details $details
        return $false
    }
    catch {
        Write-NetworkOptimizationLog "$Description failed: $($_.Exception.Message)" 'WARN'
        Add-NetworkOptimizationResult -Action $Description -Success $false -Details $_.Exception.Message
        return $false
    }
}
...
function Backup-NetworkOptimizationConfiguration {
    ...
    try {
        ...
        $backupPath = Join-Path -Path $BackupRoot -ChildPath ("NetworkBackup_{0}" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
        New-Item -Path $backupPath -ItemType Directory -Force | Out-Null

        Get-NetAdapter -ErrorAction Stop | Export-Clixml -LiteralPath (Join-Path $backupPath 'NetAdapter.xml')
        Get-NetIPConfiguration -ErrorAction Stop | Export-Clixml -LiteralPath (Join-Path $backupPath 'NetIPConfiguration.xml')
        Get-DnsClientServerAddress -ErrorAction Stop | Export-Clixml -LiteralPath (Join-Path $backupPath 'DnsClientServerAddress.xml')
        Get-NetIPInterface -ErrorAction Stop | Export-Clixml -LiteralPath (Join-Path $backupPath 'NetIPInterface.xml')

        Invoke-NetworkExternalCommand -FilePath 'reg.exe' -ArgumentList @('export','HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters',(Join-Path $backupPath 'Tcpip_Parameters.reg'),'/y') -Description 'Backup TCP/IP registry settings' | Out-Null
        Invoke-NetworkExternalCommand -FilePath 'reg.exe' -ArgumentList @('export','HKLM\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters',(Join-Path $backupPath 'Tcpip6_Parameters.reg'),'/y') -Description 'Backup TCP/IPv6 registry settings' | Out-Null

        Write-NetworkOptimizationLog "Network configuration backup saved to: $backupPath" 'OK'
        Add-NetworkOptimizationResult -Action 'Network configuration backup' -Success $true -Details $backupPath
        return $backupPath
    }
    catch {
        Write-NetworkOptimizationLog "Network configuration backup failed: $($_.Exception.Message)" 'WARN'
        Add-NetworkOptimizationResult -Action 'Network configuration backup' -Success $false -Details $_.Exception.Message
        return $null
    }
}

function Set-NetworkOptimizationIPv6Disabled {
    [CmdletBinding()]
    param()

    Write-NetworkOptimizationLog 'Applying campus baseline: disable IPv6 on active physical adapters and via DisabledComponents registry value.' 'INFO'

    try {
        $tcpip6Path = 'HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters'
        if (-not (Test-Path -LiteralPath $tcpip6Path)) {
        ...
        Write-NetworkOptimizationLog 'Set IPv6 DisabledComponents to 0xff.' 'OK'
        Add-NetworkOptimizationResult -Action 'Disable IPv6 registry baseline' -Success $true -Details 'DisabledComponents=0xff'
    }
    catch {
        Write-NetworkOptimizationLog "Failed to set IPv6 registry baseline: $($_.Exception.Message)" 'ERROR'
        Add-NetworkOptimizationResult -Action 'Disable IPv6 registry baseline' -Success $false -Details $_.Exception.Message
    }
    ...
    foreach ($adapter in $adapters) {
        try {
            ...
            if ($binding -and $binding.Enabled) {
                ...
                Write-NetworkOptimizationLog "Disabled IPv6 binding on adapter: $($adapter.Name)" 'OK'
                Add-NetworkOptimizationResult -Action "Disable IPv6 binding - $($adapter.Name)" -Success $true
            }
            elseif ($binding) {
                Write-NetworkOptimizationLog "IPv6 binding already disabled on adapter: $($adapter.Name)" 'SKIP'
                Add-NetworkOptimizationResult -Action "Disable IPv6 binding - $($adapter.Name)" -Success $true -Details 'Already disabled'
            }
        }
        catch {
            Write-NetworkOptimizationLog "Failed to disable IPv6 binding on $($adapter.Name): $($_.Exception.Message)" 'WARN'
            Add-NetworkOptimizationResult -Action "Disable IPv6 binding - $($adapter.Name)" -Success $false -Details $_.Exception.Message
        }
    }
}

function Set-NetworkOptimizationDynamicDns {
    [CmdletBinding()]
    param()

    Write-NetworkOptimizationLog 'Setting DNS server configuration to dynamic/DHCP. No public DNS servers will be configured.' 'INFO'

    $interfaces = Get-DnsClientServerAddress -AddressFamily IPv4 -ErrorAction SilentlyContinue |
        Where-Object { $_.InterfaceAlias -and $_.InterfaceAlias -notmatch 'Loopback|vEthernet|Virtual|VPN|TAP|Pseudo' } |
        Sort-Object -Property InterfaceAlias -Unique

    foreach ($interface in $interfaces) {
        try {
            Set-DnsClientServerAddress -InterfaceIndex $interface.InterfaceIndex -ResetServerAddresses -ErrorAction Stop
            Write-NetworkOptimizationLog "DNS server assignment reset to dynamic on: $($interface.InterfaceAlias)" 'OK'
            Add-NetworkOptimizationResult -Action "Dynamic DNS - $($interface.InterfaceAlias)" -Success $true
        }
        catch {
            Write-NetworkOptimizationLog "Failed to reset DNS to dynamic on $($interface.InterfaceAlias): $($_.Exception.Message)" 'WARN'
            Add-NetworkOptimizationResult -Action "Dynamic DNS - $($interface.InterfaceAlias)" -Success $false -Details $_.Exception.Message
        }
    }

    try {
        Clear-DnsClientCache -ErrorAction Stop
        Write-NetworkOptimizationLog 'DNS client cache cleared.' 'OK'
        Add-NetworkOptimizationResult -Action 'Clear DNS client cache' -Success $true
    }
    catch {
        Invoke-NetworkExternalCommand -FilePath 'ipconfig.exe' -ArgumentList @('/flushdns') -Description 'Flush DNS cache with ipconfig' | Out-Null
    }

    Invoke-NetworkExternalCommand -FilePath 'ipconfig.exe' -ArgumentList @('/registerdns') -Description 'Register DNS client records' | Out-Null
}

function Set-NetAdapterAdvancedPropertyIfSupported {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$AdapterName,
        [Parameter(Mandatory)][string[]]$DisplayNamePatterns,
    ...
    foreach ($pattern in $DisplayNamePatterns) {
        ...
        foreach ($property in $properties) {
            $validValues = @()
            try {
                if ($property.ValidDisplayValues) { $validValues = @($property.ValidDisplayValues) }
            }
            catch { $validValues = @() }

            $targetValue = $null
            foreach ($candidate in $PreferredValues) {
                if ($validValues.Count -eq 0 -or ($validValues -contains $candidate)) {
            ...
            if ([string]::IsNullOrWhiteSpace($targetValue)) {
                Write-NetworkOptimizationLog "$Description skipped on $AdapterName because no supported target value was found for '$($property.DisplayName)'." 'SKIP'
                continue
            }

            try {
                if ($property.DisplayValue -eq $targetValue) {
                    Write-NetworkOptimizationLog "$Description already set on $AdapterName ($($property.DisplayName) = $targetValue)." 'SKIP'
                    Add-NetworkOptimizationResult -Action "$Description - $AdapterName" -Success $true -Details 'Already configured'
                }
                else {
                    ...
                    Write-NetworkOptimizationLog "$Description set on $AdapterName ($($property.DisplayName) = $targetValue)." 'OK'
                    Add-NetworkOptimizationResult -Action "$Description - $AdapterName" -Success $true -Details "$($property.DisplayName)=$targetValue"
                }
            }
            catch {
                Write-NetworkOptimizationLog "$Description failed on $AdapterName for '$($property.DisplayName)': $($_.Exception.Message)" 'WARN'
                Add-NetworkOptimizationResult -Action "$Description - $AdapterName" -Success $false -Details $_.Exception.Message
            }
        }
    }
}

function Optimize-NetworkAdapterModernSettings {
    [CmdletBinding()]
    param()

    Write-NetworkOptimizationLog 'Optimizing supported Ethernet and Wi-Fi adapter settings.' 'INFO'

    $adapters = Get-NetAdapter -Physical -ErrorAction SilentlyContinue | Where-Object { $_.Status -ne 'Disabled' }
    if (-not $adapters) {
        Write-NetworkOptimizationLog 'No enabled physical network adapters were found.' 'WARN'
        return
    }
    ...
    foreach ($adapter in $adapters) {
        ...
        $isWireless = ($description -match 'Wireless|Wi-Fi|WiFi|802\.11|WLAN') -or ($name -match 'Wi-Fi|Wireless|WLAN')
        $adapterType = if ($isWireless) { 'Wi-Fi' } else { 'Ethernet' }

        Write-NetworkOptimizationLog "Processing $adapterType adapter: $name [$description]" 'INFO'

        try {
            Set-NetIPInterface -InterfaceAlias $name -AutomaticMetric Enabled -ErrorAction Stop
            Write-NetworkOptimizationLog "Automatic interface metric enabled on $name." 'OK'
            Add-NetworkOptimizationResult -Action "Automatic metric - $name" -Success $true
        }
        catch {
            Write-NetworkOptimizationLog ("Failed to enable automatic metric on {0}: {1}" -f $name, $_.Exception.Message) 'WARN'
            Add-NetworkOptimizationResult -Action "Automatic metric - $name" -Success $false -Details $_.Exception.Message
        }

        try {
            Enable-NetAdapterRss -Name $name -ErrorAction Stop
            Write-NetworkOptimizationLog "RSS enabled on $name." 'OK'
            Add-NetworkOptimizationResult -Action "Enable RSS - $name" -Success $true
        }
        catch {
            Write-NetworkOptimizationLog "RSS not enabled on $name. Adapter may not support it: $($_.Exception.Message)" 'SKIP'
        }

        try {
            Enable-NetAdapterRsc -Name $name -ErrorAction Stop
            Write-NetworkOptimizationLog "RSC enabled on $name." 'OK'
            Add-NetworkOptimizationResult -Action "Enable RSC - $name" -Success $true
        }
        catch {
            Write-NetworkOptimizationLog "RSC not enabled on $name. Adapter may not support it: $($_.Exception.Message)" 'SKIP'
        }

        try {
            Enable-NetAdapterChecksumOffload -Name $name -ErrorAction Stop
            Write-NetworkOptimizationLog "Checksum offload enabled on $name." 'OK'
            Add-NetworkOptimizationResult -Action "Enable checksum offload - $name" -Success $true
        }
        catch {
            Write-NetworkOptimizationLog "Checksum offload not enabled on $name. Adapter may not support it: $($_.Exception.Message)" 'SKIP'
        }

        try {
            Enable-NetAdapterLso -Name $name -ErrorAction Stop
            Write-NetworkOptimizationLog "Large Send Offload enabled on $name." 'OK'
            Add-NetworkOptimizationResult -Action "Enable LSO - $name" -Success $true
        }
        catch {
            Write-NetworkOptimizationLog "Large Send Offload not enabled on $name. Adapter may not support it: $($_.Exception.Message)" 'SKIP'
        }

        Set-NetAdapterAdvancedPropertyIfSupported -AdapterName $name -DisplayNamePatterns @('*Energy Efficient Ethernet*','*EEE*','*Green Ethernet*','*Power Saving Mode*','*System Idle Power Saver*') -PreferredValues @('Disabled','Off','0') -Description 'Disable NIC energy-saving features'
        Set-NetAdapterAdvancedPropertyIfSupported -AdapterName $name -DisplayNamePatterns @('*Flow Control*') -PreferredValues @('Rx & Tx Enabled','Enabled','Auto Negotiation','Auto') -Description 'Enable reliable flow control where supported'

        if ($isWireless) {
            Set-NetAdapterAdvancedPropertyIfSupported -AdapterName $name -DisplayNamePatterns @('*Preferred Band*','*Band Preference*') -PreferredValues @('Prefer 5GHz band','Prefer 5GHz Band','3. Prefer 5GHz Band','Prefer 6GHz band','Prefer 6GHz Band') -Description 'Prefer faster Wi-Fi band'
            Set-NetAdapterAdvancedPropertyIfSupported -AdapterName $name -DisplayNamePatterns @('*Roaming Aggressiveness*','*Roaming Sensitivity*') -PreferredValues @('Medium','3. Medium','Medium-Low','2. Medium-low') -Description 'Set balanced Wi-Fi roaming aggressiveness'
            Set-NetAdapterAdvancedPropertyIfSupported -AdapterName $name -DisplayNamePatterns @('*Transmit Power*','*Tx Power*') -PreferredValues @('Highest','5. Highest','High') -Description 'Set Wi-Fi transmit power high'
            Set-NetAdapterAdvancedPropertyIfSupported -AdapterName $name -DisplayNamePatterns @('*MIMO Power Save Mode*','*MIMO Power Save*','*SMPS*') -PreferredValues @('No SMPS','Disabled','Off') -Description 'Disable Wi-Fi MIMO power saving'
        }
    }
}

function Optimize-WindowsTcpStackModernSettings {
    [CmdletBinding()]
    param()

    Write-NetworkOptimizationLog 'Applying modern Windows TCP stack settings.' 'INFO'

    Invoke-NetworkExternalCommand -FilePath 'netsh.exe' -ArgumentList @('int','tcp','set','global','autotuninglevel=normal') -Description 'Enable normal TCP receive window autotuning' | Out-Null
    Invoke-NetworkExternalCommand -FilePath 'netsh.exe' -ArgumentList @('int','tcp','set','global','rss=enabled') -Description 'Enable global TCP RSS' | Out-Null
    Invoke-NetworkExternalCommand -FilePath 'netsh.exe' -ArgumentList @('int','tcp','set','global','rsc=enabled') -Description 'Enable global TCP RSC' | Out-Null
    Invoke-NetworkExternalCommand -FilePath 'netsh.exe' -ArgumentList @('int','tcp','set','global','ecncapability=disabled') -Description 'Disable TCP ECN for compatibility' | Out-Null
    Invoke-NetworkExternalCommand -FilePath 'netsh.exe' -ArgumentList @('int','tcp','set','global','timestamps=disabled') -Description 'Disable TCP timestamps for compatibility' | Out-Null

    try {
        Set-NetOffloadGlobalSetting -ReceiveSegmentCoalescing Enabled -ErrorAction Stop
        Write-NetworkOptimizationLog 'Global Receive Segment Coalescing enabled.' 'OK'
        Add-NetworkOptimizationResult -Action 'Global RSC' -Success $true
    }
    catch {
        Write-NetworkOptimizationLog "Global RSC setting skipped: $($_.Exception.Message)" 'SKIP'
    }
}

function Set-NetworkAdapterPowerManagementOptimized {
    [CmdletBinding()]
    param()

    Write-NetworkOptimizationLog 'Disabling adapter power-management options that can reduce throughput or cause drops.' 'INFO'
    ...
    foreach ($adapter in $adapters) {
        try {
            ...
            if ($pm.WakeOnMagicPacket -and $pm.WakeOnMagicPacket -ne 'Enabled') {
            ...
            if ($changed) {
                ...
                Write-NetworkOptimizationLog "Power management optimized on $($adapter.Name)." 'OK'
                Add-NetworkOptimizationResult -Action "Power management - $($adapter.Name)" -Success $true
            }
            else {
                Write-NetworkOptimizationLog "Power management already optimized on $($adapter.Name)." 'SKIP'
            }
        }
        catch {
            Write-NetworkOptimizationLog "Power management setting skipped on $($adapter.Name): $($_.Exception.Message)" 'SKIP'
        }
    }
}

function Test-NetworkOptimizationConnectivity {
    [CmdletBinding()]
    param()

    Write-NetworkOptimizationLog 'Running post-optimization connectivity checks.' 'INFO'

    $results = New-Object System.Collections.Generic.List[object]

    $checks = @(
        @{ Name = 'Default gateway'; ComputerName = $null; UseGateway = $true },
        @{ Name = 'Internet IPv4'; ComputerName = '1.1.1.1'; UseGateway = $false },
        @{ Name = 'DNS resolution'; ComputerName = 'microsoft.com'; UseGateway = $false }
    )

    $gateway = $null
    try {
        $gateway = (Get-NetIPConfiguration -ErrorAction Stop | Where-Object { $_.IPv4DefaultGateway -and $_.NetAdapter.Status -eq 'Up' } | Select-Object -First 1).IPv4DefaultGateway.NextHop
    }
    catch { $gateway = $null }

    foreach ($check in $checks) {
        $target = if ($check.UseGateway) { $gateway } else { $check.ComputerName }
        if ([string]::IsNullOrWhiteSpace($target)) {
            Write-NetworkOptimizationLog "$($check.Name) check skipped because no target was available." 'SKIP'
            continue
        }

        try {
            $ok = Test-NetConnection -ComputerName $target -InformationLevel Quiet -WarningAction SilentlyContinue -ErrorAction SilentlyContinue
            $level = if ($ok) { 'OK' } else { 'WARN' }
            Write-NetworkOptimizationLog "$($check.Name) check to $target result: $ok" $level
            $results.Add([pscustomobject]@{ Check = $check.Name; Target = $target; Success = [bool]$ok }) | Out-Null
        }
        catch {
            Write-NetworkOptimizationLog "$($check.Name) check failed: $($_.Exception.Message)" 'WARN'
            $results.Add([pscustomobject]@{ Check = $check.Name; Target = $target; Success = $false }) | Out-Null
        }
    }
    ...
    try {
        ...
        if ($computerSystem.PartOfDomain) {
            Invoke-NetworkExternalCommand -FilePath 'nltest.exe' -ArgumentList @('/dsgetdc:{0}' -f $computerSystem.Domain) -Description "Locate domain controller for $($computerSystem.Domain)" | Out-Null
            try {
                $secureChannel = Test-ComputerSecureChannel -ErrorAction Stop
                Write-NetworkOptimizationLog "Domain secure channel status: $secureChannel" ($(if ($secureChannel) { 'OK' } else { 'WARN' }))
                Add-NetworkOptimizationResult -Action 'Domain secure channel validation' -Success ([bool]$secureChannel)
            }
            catch {
                Write-NetworkOptimizationLog "Domain secure channel validation skipped or failed: $($_.Exception.Message)" 'WARN'
            }
        }
    }
    catch {
        Write-NetworkOptimizationLog "Domain detection skipped: $($_.Exception.Message)" 'SKIP'
    }

    return $results
}

function Show-NetworkOptimizationSummary {
    [CmdletBinding()]
    param(
        [string]$BackupPath,
    ...
    Write-NetworkOptimizationLog ''
    Write-NetworkOptimizationLog '========== Option 8 Network Optimization Summary ==========' 'INFO'
    if ($BackupPath) { Write-NetworkOptimizationLog "Backup path: $BackupPath" 'INFO' }
    Write-NetworkOptimizationLog 'DNS server configuration: Dynamic / DHCP-provided campus DNS.' 'INFO'
    Write-NetworkOptimizationLog 'IPv6 configuration: Disabled by adapter binding and DisabledComponents registry baseline.' 'INFO'
    Write-NetworkOptimizationLog 'Jumbo frames: Not enabled to avoid switch/path MTU mismatch.' 'INFO'

    $successCount = ($Script:NetworkOptimizationResults | Where-Object { $_.Success }).Count
    $failureCount = ($Script:NetworkOptimizationResults | Where-Object { -not $_.Success }).Count
    Write-NetworkOptimizationLog "Actions succeeded: $successCount" 'OK'
    if ($failureCount -gt 0) {
        Write-NetworkOptimizationLog "Actions failed or warned: $failureCount" 'WARN'
    }
    else {
        Write-NetworkOptimizationLog 'No failed actions were recorded.' 'OK'
    }

    if ($ConnectivityResults) {
        foreach ($result in $ConnectivityResults) {
            Write-NetworkOptimizationLog ("Connectivity: {0} -> {1} = {2}" -f $result.Check, $result.Target, $result.Success) ($(if ($result.Success) { 'OK' } else { 'WARN' }))
        }
    }

    Write-NetworkOptimizationLog "Detailed log: $Script:NetworkOptimizationLogPath" 'INFO'
    Write-NetworkOptimizationLog 'A reboot is recommended after IPv6 binding, offload, or adapter advanced-property changes.' 'WARN'
}

function Start-NetworkOptimization {
    <#
    .SYNOPSIS
        Option 8 - Optimizes Ethernet and Wi-Fi settings for managed Windows workstations.

    .DESCRIPTION
        Applies a domain-safe network baseline: DNS stays dynamic, IPv6 is disabled per campus standard,
        modern TCP/adapter offloads are enabled where supported, adapter power-saving features are reduced,
        and connectivity/domain checks are performed after configuration.
    #>
    [CmdletBinding()]
    param(
        [string]$LogDirectory = (Get-ComptonLogDirectory)
    ...
    Write-NetworkOptimizationLog 'Starting Option 8 - Network Optimization.' 'INFO'

    if (-not (Test-NetworkOptimizationAdministrator)) {
        Write-NetworkOptimizationLog 'Administrator privileges are required for Option 8.' 'ERROR'
        $global:LastStatus = '[ERROR] Option 8 Network Optimization requires Administrator privileges.'
        return
    }

    $backupPath = Backup-NetworkOptimizationConfiguration -BackupRoot $LogDirectory

    Set-NetworkOptimizationIPv6Disabled
    Set-NetworkOptimizationDynamicDns
    Optimize-WindowsTcpStackModernSettings
    Optimize-NetworkAdapterModernSettings
    Set-NetworkAdapterPowerManagementOptimized

    $connectivityResults = Test-NetworkOptimizationConnectivity
    Show-NetworkOptimizationSummary -BackupPath $backupPath -ConnectivityResults $connectivityResults

    $failedCount = ($Script:NetworkOptimizationResults | Where-Object { -not $_.Success }).Count
    if ($failedCount -gt 0) {
        $global:LastStatus = "[WARN] Option 8 completed with $failedCount warning/failure item(s). Review $Script:NetworkOptimizationLogPath. Reboot recommended."
    }
    else {
        $global:LastStatus = "[OK] Option 8 Network Optimization completed successfully. Reboot recommended. Log: $Script:NetworkOptimizationLogPath"
    }
}
...
function Show-Menu {
    ...
    Write-Host "15. Enable Automatic Login with CC-Student" -ForegroundColor White
...
function Main {
    do {
        ...
        switch ($selection) {
			...
			"17" {
				Clear-Host
				# Option 17 should only run the Full System Update workflow.
				# Do not call the default Run-CorePostDeploymentTasks task set, because the default includes
				# other menu-option functions such as bloatware removal, registry settings, services,
				# PowerShell remoting, and time sync.
